...
const LearnSocket: FC = () => {
  const socket = useMemo(
    () => io({ path: "/api/learnsocket", addTrailingSlash: false }),
    []
  );
  ...
  return (
    <>
      <div className="h-screen flex justify-center items-center px-[10vh] py-[5vh] bg-gray-100 ">
        <div className="container max-w-2xl mx-auto border-2 border-gradient-to-r from-indigo-500 to-blue-500 bg-white rounded-lg">
          ...
          <div className="container mx-auto h-[80vh] p-2 flex flex-col gap-2 overflow-y-auto flex-grow">
            {allChats.map((chat) => (
              <div className="chat-bubble self-start px-5 text-white py-2 bg-gradient-to-r from-indigo-500 to-blue-500 rounded-ss-lg rounded-se-lg rounded-ee-lg">
                {chat}
              </div>
            ))}
          </div>
          <form onSubmit={handleSendMessage}>
            <div className="container-screen flex w-full py-2 ">
              <div className="flex w-full bg-white border-t-2 border-indigo-500 p-3">
                ...
                <button
                  type="submit"
                  className=" p-3 bg-gradient-to-r from-indigo-500 to-blue-500 text-white rounded-r-lg"
                >
                  Send
                </button>
              </div>
